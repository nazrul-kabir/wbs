...
if (isset($_POST['clients_name'])) {
    ...
    $clients_link = validateInput($clients_link);
    $clients_status = validateInput($clients_status);

    // check clients exists
    $sql_check = "SELECT * FROM clients WHERE clients_name = '$clients_name'";
    $result_check = mysqli_query($con, $sql_check);
    $count = mysqli_num_rows($result_check);
    if ($count > 0) {
        $error = "Clients name already exists in record";
    } else {
        ...
        $custom_array .= ',clients_created_on = "' . $clients_created_on . '"';
        $custom_array .= ',clients_created_by = "' . $clients_created_by . '"';

        $sql = "INSERT INTO clients SET $custom_array";
        ...
        <script>
            $(document).ready(function () {
                $("#btnSave").click(function () {
                    ...
                    var clients_image = $('input[type="file"]').val();
